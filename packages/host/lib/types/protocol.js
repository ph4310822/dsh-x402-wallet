...
export const DEFAULT_HISTORY_BLOCK_RANGE = 200000n;
            ...
            for (let start = 0; start < ranges.length && entries.length < limit; start += HISTORY_WAVE) {
                ...
                const settled = await Promise.all(wave.map(async ({ from, to }) => {
                    const [outLogs, inLogs] = await Promise.all([
                        publicClient.getLogs({ address: asset.address, event: TRANSFER_EVENT, args: { from: address }, fromBlock: from, toBlock: to }),
                        publicClient.getLogs({ address: asset.address, event: TRANSFER_EVENT, args: { to: address }, fromBlock: from, toBlock: to }),
                    ]);
                    return [
