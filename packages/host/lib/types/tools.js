...
function requireAgent(exec) {
    if (exec.agent === undefined)
        throw new Error('x402 tools require an Agent-backed session');
    return exec.agent;
}
 ...
 */
export function registerX402Tools(ctx, service) {
    ctx.tools.register(defineTool({
        name: 'x402_discover',
        description: 'List live x402-enabled paid APIs from the catalog. Optionally filter by keyword (matched against '
    ...
    ctx.tools.register(defineTool({
        name: 'x402_estimate',
        description: 'Probe one URL for its x402 payment requirement without paying. A free API answers 200 and needs no '
    ...
    ctx.tools.register(defineTool({
        name: 'x402_balance',
        description: 'Read the payment wallet: whether it is configured, its address, and its USDC balance on the payment '
    ...
    ctx.tools.register(defineTool({
        name: 'x402_pay',
        description: 'Pay and call one x402-enabled URL in one step. Probes the URL, enforces maxCostUsdc (the call aborts '
