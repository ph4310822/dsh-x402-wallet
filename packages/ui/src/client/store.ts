...
import type {
  X402HistoryEntry, X402PaymentRecord, X402SendReceipt, X402WalletRecord, X402WalletState,
} from '@danielng23/dsh-x402/types'

/** One modal screen. */
export type X402WalletView = { kind: 'main' } | { kind: 'receive' } | { kind: 'send' } | { kind: 'create' }
 ...
 */
export function createX402PanelStore(): X402PanelStore {
  return defineStore({
    init: (): X402PanelState => ({
      wallet: null,
      wallets: [],
